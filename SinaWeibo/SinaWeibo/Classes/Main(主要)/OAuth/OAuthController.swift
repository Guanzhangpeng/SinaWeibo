...
extension OAuthController : UIWebViewDelegate
{
    ...
    fileprivate func getUserInfo(account :UserAccount)
    {
        ...
        NetWorkTools.Requst(methodType: .GET, urlString: "https://api.weibo.com/2/users/show.json", parameters: ["access_token":access_tocken as AnyObject, "uid":uid as AnyObject ]) { (result) in
            ...
            UserAccountViewModal.shareInstance.userAccount = account
            //序列化对象
            NSKeyedArchiver.archiveRootObject(account, toFile: UserAccountViewModal.shareInstance.filePath)
            
            STWLog(UserAccountViewModal.shareInstance.filePath)
